...
import { gymDescriptionFor } from "./gym-description";
...
async function main(): Promise<void> {
  ...
  for (const gid of ids) {
    ...
    // A gym workout's description is generated by soma, not stored on the Garmin summary, so it
    // is fetched rather than read; see gym-description.ts for why it cannot be read. Every other
    // type keeps whatever Garmin holds.
    let description = isKite
      ? generateKiteStravaDescription(summary, kite)
      : String(summary.description || "");
    let descNote = "";
    if (!isKite && summary.activityType?.typeKey === "strength_training") {
      const gym = await gymDescriptionFor(db, gid);
      if (gym.description) description = gym.description;
      else descNote = ` (gym description unavailable: ${gym.note}, keeping the stored text)`;
    }
    ...
    console.log(`  prepared ${gid} -> strava/${stravaId}: "${title}" (image=${imagePath ? `${kind} card` : `no: ${imageNote}`})${descNote}`);
  }
